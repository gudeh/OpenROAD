...
Annealer::Annealer(utl::Logger* logger,
                   ...
                   odb::dbDatabase* db,
                   Optdp* dpo)
    : logger_(logger), opendp_(opendp), network_(network), db_(db), dpo_(dpo)
{
}
...
bool Annealer::swapNodes(std::vector<Node*> small_nodes, Equivalence entry)
{
  ...
  auto big_node = network_->createAndAddNode();
  dpo_->initInstNode(big_inst, big_node->getId());
  ...
  for (auto iterm : big_inst->getITerms()) {
    ...
    dpo_->addEdgePins(iterm, iterm->getNet());
  }
  for (auto node : small_nodes) {
    dismantleNode(node);
  }
