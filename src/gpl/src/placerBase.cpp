...
void PlacerBaseCommon::initiateBtermsPosition()
{
  ...
  for (const auto& [status, count] : bterm_status_counts) {
    ...
    log_->report("GPL IO pin placement type: {:<25} = {}", label, count);
  }
...
void Net::updateBox(utl::Logger* log, bool ignore_bterms)
{
  ...
  for (Pin* pb_pin : pins_) {
    if (!pb_pin) {
      log->report("PB Net '{}' has null pin in pins_ vector", net_->getName());
      continue;
    }
